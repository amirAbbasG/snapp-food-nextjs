...
export async function getServerSideProps(context) {
  ...
  const {
    data: { shopDetails },
  } = await getShopDetailsApi(shopId);
  return {
    props: {
      shopDetails,
    },
...
const ShopDetails = ({ shopDetails }) => {
  ...
  const handlePressFavorite = async () => {
    try {
      const {
        data: { favourite },
      } = await addOrRemoveFavoriteShopApi(shopId);
            ...
            <Paper sx={{ borderRadius: 2, overflowY: "scroll", width: "100%" }}>
              ...
              <Grid container item width="100%">
                {shopDetails.foods.map((f) => (
                  <Grid key={f._id} item xs={12} md={6} p={0.4} sx={foodItem}>
            ...
            <Paper sx={deliveryCost}>
              <Container>
                ...
                <Typography
                  ...
                  component="h3"
                  mr={2}
                >{`پیک فروشنده ${cost} تومان`}</Typography>
              </Container>
            </Paper>
